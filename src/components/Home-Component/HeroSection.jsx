...
const HeroSection = () => {
  ...
  const priceChangeIcon =
    priceChange.percentage >= 0 ? <FiArrowUpRight className="inline" /> : <FiArrowDownRight className="inline" />;
          ...
          <div>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="mb-12">
              ...
              <p className="text-lg text-gray-200 mb-8 max-w-lg leading-relaxed">
                Harness real-time market intelligence, sophisticated analytics, and institutional-grade trading tools. Our
                platform transforms complex market data into actionable insights for investors at every level.
              </p>
          ...
          <div>
            ...
            >
                  ...
                  {['1h', '24h', '7d', '30d', '90d'].map((time) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs ${
                        timeframe === time ? 'bg-amber-700 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'
                      }`}
              ...
              <div className="h-80 relative">
                {isLoading ? (
                  <div className="h-full flex items-center justify-center">
                    <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }} className="text-red-400">
                      <FiRefreshCw size={40} />
                    </motion.div>
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={btcData}>
                      ...
                      />
                      <Area type="monotone" dataKey="price" stroke="#ef4444" strokeWidth={3} fillOpacity={1} fill="url(#colorPrice)" activeDot={{ r: 6, fill: '#dc2626' }} />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
              ...
              <div className="flex justify-between items-center mt-6">
                <div>
                  <div className="text-3xl font-bold">{currentPrice ? `$${currentPrice.toLocaleString()}` : 'Loading...'}</div>
                ...
                <div className="text-right">
                  ...
                  <div className="font-bold text-xl">$42.8B</div>
                </div>
              </div>
            </motion.div>
                  ...
                  <tbody>
                    {stocksData.map((stock) => (
                      <tr key={stock.symbol} className="border-b border-gray-800 hover:bg-gray-800/50 transition-colors">
                        ...
                        <td className={`py-3 text-right font-bold ${stock.change.includes('+') ? 'text-green-400' : 'text-red-400'}`}>
                          {stock.change}
                        </td>
